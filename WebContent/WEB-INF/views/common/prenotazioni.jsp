...
<body><%@ include file="/WEB-INF/views/common/navbar.jsp" %><main class="section"><h1>Le mie prenotazioni</h1>
...
<tr><td><%=a == null ? p.getAttivita_id() : a.getTitolo()%></td><td><%=p.getData_evento()%></td><td><%=p.getNum_prenotati()%></td><td>&euro; <%=String.format("%.2f", p.getPrezzo_tot())%></td><td><%=p.getStato_pagamento()%></td><td><% if (p.getData_evento().isBefore(LocalDate.now())) { %><a class="button-link" href="<%=request.getContextPath()%>/common/recensione?attivitaId=<%=p.getAttivita_id()%>">Scrivi una Recensione</a><% } %></td></tr>
