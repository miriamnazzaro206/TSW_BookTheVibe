...
<main class="section">
	...
	<div class="table-wrap">
		<table id="couponAdminTable">
			...
			<tr><td><%=c.getCode_id()%></td><td><%=c.getPercentuale()%>%</td><td><%=c.getStato() ? "Attivo" : "Non Attivo"%></td><td><form method="post" action="<%=request.getContextPath()%>/admin/coupon"><input type="hidden" name="action" value="stato"><input type="hidden" name="code_id" value="<%=c.getCode_id()%>"><input type="hidden" name="stato" value="<%=!c.getStato()%>"><button type="submit"><%=c.getStato() ? "Disattiva" : "Attiva"%></button></form></td></tr>
			<% } %>
			</tbody>
		</table>
	</div>
